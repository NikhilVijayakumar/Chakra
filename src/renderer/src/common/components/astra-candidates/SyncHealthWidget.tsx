...
export interface SyncHealthStatus {
  ...
  lastIntegrityCheck: IntegrityCheckState
  lastPull: SyncOperationState
  lastPush: SyncOperationState
  queue: SyncQueueState
}

export interface SyncHealthWidgetProps {
  syncStatus: SyncHealthStatus | null
  isLoading: boolean
    ...
    >
      ...
      >
        ...
        <Button
          ...
          variant="outlined"
          onClick={onRefresh}
          disabled={isLoading || isSyncActionRunning}
        >
        ...
        <>
          ...
          >
            <Box>
              ...
              >
                {literal['settings.sync.health.lastRun']}:{' '}
                {formatTimestamp(syncStatus.lastPull.at, literal['global.na'])}
              </Typography>
              <Typography
                variant="caption"
                sx={{ display: 'block', color: muiTheme.palette.text.secondary }}
            ...
            <Box>
              ...
              >
                {literal['settings.sync.health.lastRun']}:{' '}
                {formatTimestamp(syncStatus.lastPush.at, literal['global.na'])}
              </Typography>
              <Typography
                variant="caption"
                sx={{ display: 'block', color: muiTheme.palette.text.secondary }}
            ...
            <Box sx={{ mt: spacing.md }}>
              ...
              {syncStatus.lastIntegrityCheck.issues.map((issue) => (
                <Typography
                  key={issue}
                  variant="caption"
                  sx={{ display: 'block', color: muiTheme.palette.error.main }}
                >
                  {issue}
                </Typography>
              ))}
          ...
          <Box sx={{ display: 'flex', gap: spacing.sm, mt: spacing.lg }}>
            <Button
              variant="contained"
              onClick={onPullNow}
              disabled={isSyncActionRunning || isLoading}
            >
